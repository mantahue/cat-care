...
export default function MedicalPage() {
  ...
  const [form, setForm] = useState({ visit_date: format(new Date(), 'yyyy-MM-dd'), reason: '', diagnosis: '', instructions: '', tags: [], photo_url: null })
  const [uploading, setUploading] = useState(false)
  const [suppForm, setSuppForm] = useState({ name: '', frequency: '每日', dosage: '', notes: '' })

  useEffect(() => {
    if (!configured) return
    loadRecords(); loadSupps()
  }, [])
  ...
  const openModal = () => {
    setForm({ visit_date: format(new Date(), 'yyyy-MM-dd'), reason: '', diagnosis: '', instructions: '', tags: [], photo_url: null })
    setModal(true)
  }
  ...
  const handlePhoto = async (e) => {
    ...
    const { data, error } = await supabase.storage.from('cat-photos').upload(path, file)
    if (!error) {
    ...
    <div className="pb-nav fade-in" style={{ padding: '0 14px 14px' }}>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--brown-l)', letterSpacing: .5 }}>目前保健品</span>
          <button onClick={() => { setSuppForm({ name: '', frequency: '每日', dosage: '', notes: '' }); setSuppModal(true) }}
            style={{ background: 'var(--surface)', border: 'none', borderRadius: 8, padding: '5px 10px', fontSize: 12, fontWeight: 700, cursor: 'pointer', color: 'var(--text-s)', display: 'flex', alignItems: 'center', gap: 4 }}>
            <Plus size={12} /> 新增
          </button>
        </div>
        {supps.length === 0 ? (
          ...
          <div>
            {supps.map(s => (
              <SwipeDelete key={s.id} onDelete={() => deleteSupp(s.id)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 0', borderBottom: '0.5px solid var(--surface)' }}>
      ...
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4, marginBottom: 10 }} className="scrollbar-hide">
        <button onClick={() => setFilterTag(null)} style={{
          ...
          borderColor: !filterTag ? 'var(--orange-d)' : 'var(--surface)',
          background: !filterTag ? 'var(--orange-d)' : 'var(--cream)',
          color: !filterTag ? 'white' : 'var(--text-s)',
          fontSize: 11, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap',
        }}>全部</button>
        {ALL_TAGS.map(t => (
          <button key={t} onClick={() => setFilterTag(filterTag === t ? null : t)} style={{
            padding: '5px 12px', borderRadius: 999, border: '1.5px solid',
            borderColor: filterTag === t ? 'var(--orange-d)' : 'var(--surface)',
            background: filterTag === t ? 'var(--orange-d)' : 'var(--cream)',
            color: filterTag === t ? 'white' : 'var(--text-s)',
            fontSize: 11, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap',
          }}>{t}</button>
        ))}
      </div>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--brown-l)', letterSpacing: .5 }}>看診紀錄</span>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((r, i) => (
            <SwipeDelete key={r.id} onDelete={() => deleteRecord(r.id)}>
              <div style={{ padding: '12px 0', borderBottom: i < filtered.length - 1 ? '0.5px solid var(--surface)' : 'none' }}>
                <div style={{ fontSize: 11, color: 'var(--brown-l)', fontWeight: 600 }}>{r.visit_date?.replace(/-/g, '/')}</div>
                <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--text)', margin: '3px 0' }}>{r.reason}</div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="🏥 新增看診紀錄">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
              {ALL_TAGS.map(t => (
                <button key={t} onClick={() => form.tags.includes(t) ? removeTag(t) : addTag(t)} style={{
                  padding: '4px 10px', borderRadius: 999,
                  background: form.tags.includes(t) ? 'var(--orange-d)' : 'var(--surface)',
                  color: form.tags.includes(t) ? 'white' : 'var(--text-s)',
          ...
          <div>
            ...
            {form.photo_url ? (
              ...
              <label style={{
                ...
                borderRadius: 12, border: '1.5px dashed var(--surface)',
                cursor: 'pointer', color: 'var(--text-s)', fontSize: 13,
              }}>
                <Camera size={16} />
          ...
          <button onClick={saveRecord} style={{
            width: '100%', padding: 14, borderRadius: 14, background: 'var(--orange-d)',
            color: 'white', border: 'none', fontSize: 15, fontWeight: 800, cursor: 'pointer',
          }}>儲存看診紀錄</button>
        </div>
      </Modal>

      <Modal open={suppModal} onClose={() => setSuppModal(false)} title="💊 新增保健品">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
