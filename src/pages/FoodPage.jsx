...
export default function FoodPage() {
  ...
  const [form, setForm] = useState({ category: '飼料', brand: '', name: '', rating: '愛吃', is_current: false, notes: '' })
  ...
  const toggleCurrent = async (item) => {
    const updated = { ...item, is_current: !item.is_current }
    if (configured) await supabase.from('food_items').update({ is_current: updated.is_current }).eq('id', item.id)
    ...
    <div className="pb-nav fade-in" style={{ padding: '0 14px 14px' }}>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--brown-l)', letterSpacing: .5 }}>
            {activeTab}紀錄（{filtered.length}）
          </span>
          <button onClick={() => { setForm({ category: activeTab, brand: '', name: '', rating: '愛吃', is_current: false, notes: '' }); setModal(true) }}
        ...
        {filtered.length === 0 ? (
          ...
          return (
            <SwipeDelete key={item.id} onDelete={() => del(item.id)}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '0.5px solid var(--surface)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {item.notes && <div style={{ fontSize: 11, color: item.notes.includes('吐') ? 'var(--red)' : 'var(--text-s)', marginTop: 2 }}>{item.notes}</div>}
                </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={`🍽️ 新增${form.category}`}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="例如：吃完會吐、只有加水才吃…"
              style={{ width: '100%', padding: '10px 12px', borderRadius: 12, border: '1.5px solid var(--surface)', background: 'var(--cream)', fontSize: 13, color: 'var(--text)' }} />
          </div>
