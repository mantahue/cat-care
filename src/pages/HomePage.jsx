...
import { Plus, MessageSquarePlus, Send, Trash2, Droplets, Wind } from 'lucide-react'
...
const DEMO_TOILET = [
  ...
  { id: '3', type: 'pee', status: null, logged_at: new Date().toISOString().replace(/T.*/, 'T11:45:00Z'), note: '' },
]
const DEMO_LOG = { water_cc: null, water_note: '中午有去浴室喝水' }
const DEMO_MSGS = [
...
const POOP_OPTS = [
  { v: '正常',   emoji: '✅', badColor: false },
  { v: '軟便',   emoji: '⚠️', badColor: true },
  { v: '拉肚子', emoji: '🚨', badColor: true },
  { v: '便秘乾硬', emoji: '🪨', badColor: true },
]
...
export default function HomePage({ catName }) {
  ...
  const [waterCc, setWaterCc]     = useState('')
  ...
  const loadWater = async () => {
    ...
    if (data) { setWaterCc(data.water_cc ?? ''); setWaterNote(data.water_note ?? '') }
  }
  ...
  const saveToilet = async () => {
    const entry = {
      type: toiletType,
      status: toiletType === 'poop' ? poopStatus : null,
      logged_at: new Date(toiletTime).toISOString(),
      note: toiletNote || null,
    }
  ...
  const saveWater = async (cc, note) => {
    if (!configured) return
    await supabase.from('daily_logs').upsert({ log_date: today, water_cc: cc || null, water_note: note || null }, { onConflict: 'log_date' })
  }
    ...
    <div className="pb-nav fade-in" style={{ padding: '0 14px 14px' }}>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--brown-l)', letterSpacing: .5 }}>🍚 今日餵食</span>
          <button onClick={openFeedModal} className="btn-press" style={{
            background: 'var(--orange-d)', color: 'white', border: 'none', borderRadius: 10,
        ...
        {meals.length === 0 ? (
          ...
          <>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 10, flexWrap: 'wrap' }}>
              {meals.map((m, i) => (
                <button key={m.id} onClick={() => setViewMeal(m)} style={{
                  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3,
                  background: 'none', border: 'none', cursor: 'pointer', padding: 0,
                }}>
                  <div style={{
                    width: 30, height: 30, borderRadius: '50%', background: 'var(--orange)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    color: 'white', fontWeight: 800, fontSize: 13,
                    boxShadow: '0 2px 6px rgba(230,122,45,0.3)',
                  }}>{i + 1}</div>
            ...
            {meals.map(m => (
              <SwipeDelete key={m.id} onDelete={() => deleteMeal(m.id)}>
                <div onClick={() => setViewMeal(m)} style={{
                  padding: '8px 4px', borderBottom: '0.5px solid var(--surface)',
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
          <button onClick={() => openToiletModal('pee')} className="btn-press" style={{
            padding: '14px 10px', borderRadius: 14, border: 'none', cursor: 'pointer',
            background: '#E8F3F8', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
          }}>
          ...
          <button onClick={() => openToiletModal('poop')} className="btn-press" style={{
            padding: '14px 10px', borderRadius: 14, border: 'none', cursor: 'pointer',
            background: '#FFF0E0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
          }}>
            ...
            {toilet.map(t => {
              ...
              return (
                <SwipeDelete key={t.id} onDelete={() => deleteToilet(t.id)}>
                  <div onClick={() => setViewToilet(t)} style={{
                    display: 'flex', alignItems: 'center', gap: 8,
                    padding: '8px 4px', borderBottom: '0.5px solid var(--surface)', cursor: 'pointer',
                  }}>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
          <input type="number" placeholder="cc 數" value={waterCc}
            onChange={e => setWaterCc(e.target.value)}
            onBlur={() => saveWater(waterCc ? parseInt(waterCc) : null, waterNote)}
            style={{ width: 80, padding: '8px 10px', borderRadius: 10, border: '1.5px solid var(--surface)', background: 'var(--cream)', fontSize: 16, fontWeight: 700, color: 'var(--text)', textAlign: 'center' }} />
          <span style={{ fontSize: 13, color: 'var(--text-s)', fontWeight: 600 }}>cc</span>
        </div>
        <input placeholder="備註，例如：中午有去浴室喝水" value={waterNote}
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--brown-l)', marginBottom: 12 }}>💬 家人留言板</div>
        <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
          ...
          <input value={msgText} onChange={e => setMsgText(e.target.value)} placeholder="留言給家人…"
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
              {['飼料一盤', '飼料半盤', '罐頭半罐', '罐頭一罐', '零食少許'].map(t => (
                <button key={t} onClick={() => setFeedDesc(t)} style={{
                  padding: '5px 10px', borderRadius: 999, border: '1.5px solid var(--surface)',
                  background: feedDesc === t ? 'var(--orange)' : 'var(--cream)',
                  color: feedDesc === t ? 'white' : 'var(--text-s)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {[['pee', '💧 尿尿'], ['poop', '💩 便便']].map(([t, l]) => (
              <button key={t} onClick={() => setToiletType(t)} style={{
                padding: '11px 0', borderRadius: 12, border: '2px solid',
                borderColor: toiletType === t ? (t === 'pee' ? '#6BA3BE' : '#C07840') : 'transparent',
                background: toiletType === t ? (t === 'pee' ? '#E8F3F8' : '#FFF0E0') : 'var(--surface)',
                fontSize: 14, fontWeight: 700,
                color: toiletType === t ? (t === 'pee' ? '#3A6B8A' : '#8A5020') : 'var(--text-s)',
                cursor: 'pointer',
              }}>{l}</button>
            ))}
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {POOP_OPTS.map(({ v, emoji }) => {
                  const c = poopColor(v)
                  return (
                    <button key={v} onClick={() => setPoopStatus(v)} style={{
                      padding: '11px 8px', borderRadius: 12, border: '2px solid',
                      borderColor: poopStatus === v ? (v === '正常' ? '#6DB56A' : '#E85D4A') : 'transparent',
                      background: poopStatus === v ? c.bg : 'var(--surface)',
                      color: poopStatus === v ? c.text : 'var(--text-s)',
          ...
          <button onClick={saveToilet} style={{
            width: '100%', padding: 14, borderRadius: 14,
            background: toiletType === 'pee' ? '#6BA3BE' : '#C07840',
      ...
      <Modal open={!!viewToilet} onClose={() => setViewToilet(null)} title="上廁所紀錄詳細">
        {viewToilet && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {viewToilet.note && (
              <div style={{ fontSize: 13, color: 'var(--text-s)', background: 'var(--surface)', padding: '8px 12px', borderRadius: 10 }}>{viewToilet.note}</div>
            )}
